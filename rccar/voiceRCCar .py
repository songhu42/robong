...
def write_wave(q):
    paud = pa.PyAudio()
    ws = wave.open("./recode.wav", "wb")
    ws.setsampwidth(paud.get_sample_size(pa.paInt16))
    ws.setnchannels(1)
    ws.setframerate(RATE)
    ws.writeframes(b''.join(q))
...
def check_cmd(q):
    ...
    rec = util.toMFCC("./recode.wav", duration=1)
    rec = np.array(rec)
    print(rec.shape) 
