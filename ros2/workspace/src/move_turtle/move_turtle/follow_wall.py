...
class Move_turtle(Node):
    ...
    def update(self):
        ...
        if not self.find_wall:
            ...
            if self.laserscan_degree[0] < 0.2:
                self.find_wall = True

        else:
            # 처음 벽 만났을때, 코너에서
            if self.laserscan_degree[90] > 1.00:
                self.twist.linear.x = 0
                self.twist.angular.z += MAX_ANGLE / 20
            elif self.laserscan_degree[45] > 1.00:
                self.twist.linear.x = MAX_VEL/10
            ...
            # 너무 멀 때
                ...
                if self.laserscan_degree[45] > self.laserscan_degree[135]:
                    ...
                    self.twist.angular.z = -MAX_ANGLE / 8
            # 너무 가까울 때
            elif self.laserscan_degree[45]+self.laserscan_degree[135] < 0.4:
                self.twist.linear.x = MAX_VEL/4
                self.twist.angular.z = -MAX_ANGLE / 8
            # 적당한 거리 일 때
            else:
